...
// {
//     /*The Update information of existing account feature, users must be able
//     to update their country or phone number.

// 2.1. You must ask users to input the account id they want to change, followed
// by a prompt asking which field they want to also change (the only fields that
// are permitted to update is the phone number and the country).

// 2.2. Whenever users update an account, it must be saved into the
// corresponding file.*/
...
void Update_account_information(struct User u) {
  ...
  while (fscanf(pf, "%d %d %s %d %d/%d/%d %s %s %lf %s", &r.id, &r.userId,
  ...
  for (int i = 0; i < count; i++) {
    if (strcmp(userNames[i], u.name) == 0 &&
        records[i].accountNbr == accountId) {
      idx = i;
      break;
    }
  }
  ...
  for (int i = 0; i < count; i++) {
    fprintf(pf, "%d %d %s %d %d/%d/%d %s %s %.2lf %s\n\n", records[i].id,
            records[i].userId, records[i].name, records[i].accountNbr,
            records[i].deposit.month, records[i].deposit.day,
            records[i].deposit.year, records[i].country, records[i].phone,
  ...
  while (getAccountFromFile(pf, userName, &r)) {
    if (strcmp(userName, u.name) == 0 && accnbr == r.accountNbr) {
      ...
      printf("\nAccount number:%d\nDeposit Date:%d/%d/%d \ncountry:%s \nPhone "
             ...
             r.accountNbr, r.deposit.month, r.deposit.day, r.deposit.year,
             r.country, r.phone, r.amount, r.accountType);
      /*3.2. If the account is either savings, fixed01, fixed02 and fixed03 the
system will display the information of that account and the interest you will
acquire depending on the account:
...
If the account is current you must display You will not get interests because
the account is of type current

For example: for an account of type savings with a deposit date of 10/10/2002
and an amount of $1023.20 the system will show "You will get $5.97 as interest
on day 10 of every month".*/
      fclose(pf);
      float rate = r.amount;
      ...
      if (strcmp(r.accountType, "current") == 0)
        ...
        printf("You will get $%.2f as interest on day %d of every month", rate,
               r.deposit.month);
    }
  }
  success(u);
...
void Make_transaction(struct User u) {
  ...
  // Read all records into memory
  while (fscanf(pf, "%d %d %s %d %d/%d/%d %s %s %lf %s", &r.id, &r.userId,
                r.name, &r.accountNbr, &r.deposit.day, &r.deposit.month,
                &r.deposit.year, r.country, r.phone, &r.amount,
                r.accountType) == 11) {
    strcpy(userNames[count], r.name);
    records[count] = r;
  ...
  // Find the record to update
  for (int i = 0; i < count; i++) {
    // printf("checking name: %s accNum: %d", userNames[i],
    // records[i].accountNbr);
    if (strcmp(userNames[i], u.name) == 0 &&
        records[i].accountNbr == accountNbr) {
      idx = i;
      break;
    }
  }
...
void Transfer_ownership(struct User u) {
  /*The Transfer owner feature, users can transfer their account to another
  user, by:

  6.1. Identifying the account and the user they want to transfer the ownership
  to
